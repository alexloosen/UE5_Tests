...
UTP_WeaponComponent::UTP_WeaponComponent()
{
	...
	//WeaponMesh->AttachToComponent(Mesh1P,AttachmentRules, FName(TEXT("right_hand_socket")));
	//WeaponMesh->SetWorldScale3D(FVector(0.65, 0.65, 0.65));
...
void UTP_WeaponComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if(Character != nullptr)
	{
		...
		Character->OnUseSecondaryItem.RemoveDynamic(this, &UTP_WeaponComponent::SecondaryFire);

	}
}

void UTP_WeaponComponent::AttachWeapon(AUE5_TestsCharacter* TargetCharacter)
{
	...
	if(Character != nullptr)
	{
		...
		// Register so that Fire is called every time the character tries to use the item being held
		Character->OnUseSecondaryItem.AddDynamic(this, &UTP_WeaponComponent::SecondaryFire);
	}
}
